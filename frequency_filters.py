...
class FrequencyFilterApp(BaseFrame):
    def __init__(self, parent):
        ...
        content = self.create_header(
            "Frequency Domain Filtering (Pg 14-16)",
        ...
        # Tombol default: generate pattern (sekarang HITAM POLOS)
        ttk.Button(
            ctrl_frame,
            text="📄 Generate Pattern (Black Background)",
            command=self.generate_pattern_black,
        ...
        cb_shape = ttk.Combobox(
            ctrl_frame,
            textvariable=self.filter_shape,
        ...
        for r in [5, 15, 30, 80, 230]:
            tk.Button(
                ...
                font=("Segoe UI", 8),
                width=3,
                command=lambda v=r: self.set_radius(v)
        ...
        # Tombol Aksi
        ttk.Button(ctrl_frame, text="▶ Apply Filter", command=self.apply_filter, style="Primary.TButton").pack(
            fill="x", pady=5
        )
        ...
        # Init default image (BLACK)
        self.generate_pattern_black()
    ...
    # ==========================
    # PERBAIKAN UTAMA: BACKGROUND PUTIH DIHILANGKAN
    # ==========================
    def generate_pattern_black(self):
        """
        Default pattern: HITAM POLOS (tanpa kotak putih / tanpa huruf).
        """
        img = np.zeros((IMG_SIZE, IMG_SIZE), dtype=np.uint8)  # BLACK BG
    ...
    def apply_filter(self, event=None):
        ...
        if shape == "Ideal":
            ...
            H_lp = 1 / (1 + (D / D0) ** (2 * n))
            H = H_lp if mode == "Lowpass" else (1 - H_lp)
